...
308                                                                                                    III. Mathematical Concepts
      ...
      (−1,0)                    O                      P       x         their relation with the exponential function [III.25](/part-03/the-exponential-and-logarithmic-functions):
                                                                                                  z2     z3
                                                                                        ez = 1 + z + +      + ··· .
                                                                                                  2!     3!
...
and A = (1, 0) is θ, this gives the formula                              natural objects with which to do a fourier analysis
                        sin θ
...
for 0 ⩽ θ ⩽ π /2 (we do not care about what x is). This                  series
...
one is not obvious) is                                                    1    2π
               ...
               sin z = z −       +    −    + ··· .                 (2)                           ⎧
                              3!   5!   7!                                                       ⎨0 for all n, m ⩾ 0, n ≠ m,
                                                                                               =
This infinite series converges for all real z. The result-                                        ⎩1 n = m,
ing definition has a distinct advantage over (1), in that
                                                                         and
it also makes sense when z is an arbitrary complex
number (that is why we replaced the letter θ by z). It                      1 2π
                                                                                    cos nx sin mx dx = 0       for all n, m ⩾ 0.
therefore allows us to extend sin to a holomorphic                          π 0
function [I.3 §5.6](/part-01/fundamental-definitions) on C.                                                Thus, for example, we have
   If the sine function is analytic, then what is its deriva-                                  1 2π
                                                                                        an =          f (x) cos nx dx.
...
which may be obtained by differentiating the series for                   trigonometric functions (which we have not discussed
sin term by term (naturally, this is an operation that                   here), as well as integrals involving these functions. It is
