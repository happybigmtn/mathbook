...
Interpolation is useful here because the inequality is easy to prove in the extreme cases when $p = 1$ , when $q = 1$ , or when $r = \infty.$ It is much harder to prove this result without the help of interpolation theory. III . $30$
...
It is not hard to show that the integral is in fact convergent whenever s is a complex number and Re (s) (the real part of s) is positive. Moreover, it defines a holomorphic function [I.3](/part-01/fundamental-definitions) in that region. When the real part of s is negative, the integral does not converge at all, and so the formula ( $1$ ) cannot be used to define the gamma function in its entirety. However, we can instead use the property $Γ(s) = (s - 1)Γ(s - 1)$ to extend the definition.
