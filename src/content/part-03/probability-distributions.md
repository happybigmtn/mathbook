...
heads is $0$ and $a$ probability o$f^{2}$ that it is $1$ . More generally, a (discrete) sample space is simply a set $\Omega,$ and a probability distribution on $\Omega$ is a way of assigning a nonnegative real number to each element of $\Omega$ in such a way that the sum of all these numbers is $1$ . The number assigned to a particular element of $\Omega$ is then interpreted as the probability that some corresponding outcome will occur, the total probability being $1$ .
