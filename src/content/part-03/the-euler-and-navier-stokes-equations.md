...
This error estimate shows just how special the contin-ued-fraction approximations are: if you pick a denom in at orq without thinking, and then select the numera- torcan guarantee is thatp that makes p/q xclosest tolies betweenx, the only thing you(p - 1/2)/q and(pwhich is much bigger than 1+ 1/2)/q.
So the error could be as large as 1/(q2) if q is a large integer./(2 q), can have even smaller error than is guaranteed by (8).For example, the approximation Sometimes a continued-fraction approximation to$π \approx 355/113 \text{that wex}$ get by truncating (7) at the third level is exceptionally accurate, the reason being that the next partial quotient, 292, is rather large. So we are not changing the fraction much by ignoring the tail 1$/(292 + 1/(1 +$. ..)).
