...
modular arithmetic [III.58](/part-03/modular-arithmetic)). Denote by ā and \bar{b} the                  over subrings of number fields [III.63](/part-03/number-fields). (To be honest,
...
(2) is a multiple of 7, so it reduces to 0. We are left with           a subring of a number field. However, the more general
                          ā2 + \bar{b}2 = 0.                        (3)    definition suits our current purposes.)

Now there are only seven possibilities for ā, and seven                  With any particular equation like (2), one can asso-
possibilities for \bar{b}. So the analysis of the solutions of              ciate infinitely many Diophantine problems, one for
(3) amounts to checking the forty-nine choices of ā, \bar{b}               each commutative ring R. A central insight—in some
...
satisfied only if ā = \bar{b} = 0.                                          be treated as a single entity. This widening of scope
   But saying that ā = \bar{b} = 0 is the same as saying that              reveals structure that is invisible if we consider each
