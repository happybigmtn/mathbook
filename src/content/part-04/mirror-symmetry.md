...
stant, . ambda g . The power of . ambda g in the perturbation series      each operator a in one theory must get mapped to a
...
parametrization of the lattice is the ratio τ = λ2 /λ1 .        ously distinct, such as different point values of a single
  By redefining the direction of one of the . ambda s, we can           field. Mathematically, a well-established method of pro-
...
Suppose also that the target space is a circle of radius R, which we denote by S $R^{1}$ . We think of S $R^{1}$ as the real line, with two points identified if they differ by a multiple of $2\piR.$ Maps from one circle to another can be classified by their winding number, an integer that tells you how many (net) times the image of a point goes around the second circle when the point goes once around the first. The map $\theta \to mR\theta$ from the circle to (SR)1 has winding number m.
...
0 and 1 identified. Recall that the Hilbert space of states            identifying X with . ambda X for any nonzero complex num-
is constructed from (functions of) all the degrees of                 ber λ. (If X is a solution, then so is . ambda X.) The equation
...
cohomology” is called an A. nfty category. One can also imagine a cate-       ror pairs, we should set τ = ρ. The objects of the B-
