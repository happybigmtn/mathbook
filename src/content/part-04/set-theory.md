...
Moreover, the relation “p forces φ to be true,” which relates finite binary sequences to formulas and is written p   φ, can be defined in M. Therefore, to know whether a statement φ is true in M[c] one just needs to check whether there is an initial segment p of c such that p   φ. In particular, using this result one can prove that M[c] satisfies the ZFC axioms. In order to build a model where CH fails, one adds not just one generic real number but א $M^{2}$ of them, where א M of $\aleph^{2}$ in  M. That is the ordinal that plays the role
...
set. (In the case we looked at earlier, it was the set of all finite binary sequences, with p $<$ q if p was a proper initial segment of q .) If one starts with a model where GCH holds, uses only partial orderings that are CCC--that is, in which every set of incompatible elements is countable--and takes so-called direct limits at the limit stages, then in ω 2 steps one can destroy all counterexamples so that SH holds in the final model. On the other hand, Jensen proved in 1968 that a counterexample to SH exists in L, thereby proving the undecidability of SH in ZFC.
...
For example, PFA, and therefore also MM, implies the axiom of projective determinacy (PD), the singular cardinal hypothesis (SCH), and that the cardinality of R is א 2 . An advantage of forcing axioms is that one can apply them without having to go into the details of forcing, just as ♦ and   save one from having to go into the details of constructible sets.
